...
Then run the notebooks in order — no build step needed, `wise_rbdsuca.duckdb` ships
precompiled in the repo:

1. `0_prefill/Prefill_RiverBasinDistrictsAndCompetentAuthorities.ipynb` - pick a country,
   review the prefilled data and export it to SQLite + GeoPackage under `output/<CC>/`.
...
- `output/` (git-ignored) — deliverables generated locally when you run the prefill
  notebook. Nothing under here is ever committed.
- `.env` (git-ignored) — your own AI provider credentials, if you use the
